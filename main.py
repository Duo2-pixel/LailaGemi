...
GEMMA_API_KEY = os.getenv("GEMMA_API_KEY") # NEW: Specific key for Gemma
...
def save_user_name(user_id, user_name):
    try:
        ...
        existing_records = name_sheet.get_all_records()
...
async def get_bot_response(user_message: str, chat_id: int, bot_username: str, should_use_ai: bool, update: Update) -> str:
    ...
    if should_use_ai or (update.effective_chat and update.effective_chat.type == 'private'):
            ...
            try:
                ...
                if "429 Quota exceeded" in error_str or "You exceeded your current quota" in error_str or "500" in error_str:
                    ...
                    if retries == max_retries:
                        ...
                        break # Exit the loop to try Gemma
                    continue
                else:
        ...
        # --- NEW CODE: GEMMA FALLBACK using the dedicated key ---
        if GEMMA_API_KEY:
            try:
...
async def show_chats_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for chat_id in known_chats:
        try:
            ...
            else: # For groups, supergroups, and channels
                members_count = "Count not available"
                try:
                    # Get the member count for groups
                    members_count = await context.bot.get_chat_member_count(chat_id)
                except Exception:
                    # Silently handle errors if the bot can't get the count
                    pass
...
async def paid_broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Send a detailed receipt to the owner
    receipt_message = (
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not global_bot_enabled or not chat_enabled:
        return
    ...
    # --- NEW LOGIC: Check for name saving first ---
    found_name = None
    if awaiting_name[chat_id] and len(user_message.split()) == 1 and not re.search(r'\d', user_message):
        # If bot just asked for a name and user replied with a single word
        found_name = user_message.strip().title()
    else:
        # Existing logic to check for a full phrase
        name_patterns = [
            r'mera naam (.*) hai\s*',
            r'main (.*) hu(n|n)\s*',
            r'i am (.*)\s*',
    ...
    if found_name and len(found_name) > 1 and len(found_name) < 20:
    ...
    # Old logic for name saving patterns, kept for redundancy
    if not awaiting_name[chat_id]:
        name_patterns = [
            r'mera naam (.*) hai\s*',
            r'main (.*) hu(n|n)\s*',
            r'i am (.*)\s*',
        ...
        for pattern in name_patterns:
            match = re.search(pattern, user_message, re.IGNORECASE)
            if match:
                found_name_from_phrase = match.group(1).strip().title()
                if len(found_name_from_phrase) > 1 and len(found_name_from_phrase) < 20 and not re.search(r'\d', found_name_from_phrase):
                    existing_name = find_user_name(user_id)
                    if existing_name is None:
                        save_user_name(user_id, found_name_from_phrase)
                        await update.message.reply_text(f"Acha, to ab se main tumhe **{found_name_from_phrase}** bulaungi!.", parse_mode='Markdown')
                        logger.info(f"[{chat_id}] Automatically saved name for user {user_id}: '{found_name_from_phrase}'.")
                        return

    if is_private_chat or is_reply_to_bot or is_mention:
        try:
