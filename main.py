...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"[{chat_id}] /stats command used. Uptime: {uptime_str}")
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check if the bot is globally on and is either a private chat or a group that is enabled
    ...
    if not should_use_ai:
        # If it's a message in a group not meeting the above conditions, check if the AI thinks it's a question for it.
