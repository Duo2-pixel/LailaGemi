...
# --- NEW: AI-powered name extraction ---
async def get_name_from_message(user_message: str):
    """Uses AI to intelligently extract a user's name from a message."""
    prompt = f"Given the user message: '{user_message}', does it contain a name? If so, extract ONLY the name. If not, respond with 'no'."
    ...
    try:
        ...
        found_name = response.text.strip().title()
        if found_name.lower() == 'no' or len(found_name) < 2 or len(found_name) > 20:
            return None
        return found_name
    except Exception as e:
...
# --- UPDATED process_message function with AI intent check ---
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- NEW & IMPROVED: AI-powered name saving logic ---
    if not is_reply_to_bot and not is_private_chat:
        found_name = await get_name_from_message(user_message)
        if found_name:
    ...
    # --- NEW: AI-based intent check for group chats ---
    should_use_ai = is_private_chat or is_reply_to_bot
    if not should_use_ai and is_mention:
        # If it's a mention in a group, check if the message is actually a question for the bot
        if await is_message_for_laila(user_message):
            should_use_ai = True
        else:
            logger.info(f"[{chat_id}] Message for Laila was not a question. Ignoring.")
            return
    ...
    if should_use_ai:
        ...
        # For group messages not directed at the bot, just log and ignore.
