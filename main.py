...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- NEW: Logic to automatically detect and save user's name ---
    name_patterns = [
        r'mera naam (.*) hai\s*',
        r'main (.*) hu(n|n)\s*',
        r'i am (.*)\s*',
    ...
    for pattern in name_patterns:
        ...
        if match:
            ...
            # Handle Hinglish cases like "mai Ravi hu" or "I am Ravi"
            if len(found_name.split()) > 2:
                found_name = found_name.split()[0]
            if not found_name.isascii(): # Check if it's a Hindi name
                found_name = match.group(1).split()[0] if ' ' in match.group(1) else match.group(1)
            break
            
    if found_name and len(found_name) > 1 and len(found_name) < 20 and not re.search(r'\d', found_name):
    ...
    if is_private_chat or is_reply_to_bot or is_mention:
        try:
            async with context.bot.send_chat_action(chat_id=chat_id, action="typing"):
